<section class="max-w-[1130px] mx-auto" style="margin-top: 30px;">
    ...
    <div class="hero-detail">
        @if($agenda->thumbnail)
            <img src="{{ asset('storage/' . $agenda->thumbnail) }}" class="blur-bg" alt="blur background" />
            <img src="{{ asset('storage/' . $agenda->thumbnail) }}" class="main-img" alt="{{ $agenda->judul }}" />
        @else
            <div style="width: 100%; height: 100%; background: linear-gradient(135deg, #2E5BFF 0%, #042C53 100%); display: flex; align-items: center; justify-content: center; position: relative; z-index: 0;">
